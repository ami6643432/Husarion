...

{
	...
	while (ros::ok()){
		if(command_active_){
				...
				points.push_back(Eigen::Vector3d(    1.3, 0, 1.3));

				// points.push_back(Eigen::Vector3d(    0.8, 0, 0.94));

				// points.push_back(Eigen::Vector3d(    0.4, 0, 0.72));

				points.push_back(Eigen::Vector3d(    0, 0, 0.35));

				// points.push_back(Eigen::Vector3d(   -0.4, 0, 0.72));

				// points.push_back(Eigen::Vector3d(   -0.8, 0, 0.94));

				points.push_back(Eigen::Vector3d(   -1.2, 0, 1));

				// points.push_back(Eigen::Vector3d(  0.20, 0, 0.65));

				// points.push_back(Eigen::Vector3d(  0.21, 0, 0.65));

				// points.push_back(Eigen::Vector3d(  0.9, 0, 0.65));



				// points.push_back(Eigen::Vector3d( -0.9, 0, 0.75));

				// points.push_back(Eigen::Vector3d( +1.0 + xbias, ybias, 0.75));

				// points.push_back(Eigen::Vector3d( +0.2 + xbias, ybias, 0.45));
...
void start_trajectory(ros::Publisher trajectory_pub, std::vector<Eigen::Vector3d> points){
	...
	ros::Rate sleep_rate(200); //original at 100
	int i = 0;
