...
class Controller:
    ...
    def __init__(self):
        ...
        # xbias = 0.30
        # ybias =  -0.15
        # self.sp.pose.position.x = 1.0 + xbias
        # self.sp.pose.position.y = 0 + ybias
        
        self.sp.pose.position.x = 0.8
        ...
        self.desPos_m1, self.desVel_m1, self.desAcc_m1 = np.deg2rad(180), 0.1, 0.0
        self.desPos_m2, self.desVel_m2, self.desAcc_m2 = np.deg2rad(180), 0.1, 0.0
        self.desPos_m3, self.desVel_m1, self.desAcc_m1 = np.deg2rad(180), 0.1, 0.0
        # self.desPos_m4, self.desVel_m2, self.desAcc_m2 = np.deg2rad(153), 0.1, 0.0  # Gripper

        self.M1_pos_err, self.M1_vel_err, self.M2_pos_err, self.M2_vel_err = 0, 0, 0, 0
        self.M3_pos_err, self.M3_vel_err, self.M4_pos_err, self.M4_vel_err = 0, 0, 0, 0
        ...
        # self.Kp0 = np.array([1.0, 1.0, 1.0])
        # self.Kp1 = np.array([2.0, 2.0, 1.0])
        # self.Lam = np.array([1.75, 1.75, 4]) #Change 1 	#[1.75, 1.75, 4]
        # self.Phi = np.array([1.25, 1.25, 1.1])
        # self.M = 0.4 #Change 2
        # self.alpha_0 = np.array([1,1,1])
        # self.alpha_1 = np.array([3,3,3])
        # self.alpha_m = 0.05 #Change 3
        ...
        self.position_msg.position.append(0)           # ** WILL NEED TO APPEND FOR OTHER TWO MOTORS
        ...
        self.desPos_m1, self.desPos_m2 = 180, 180       # * MIGHT NEED TO EDIT THESE
    ...
    def multiDoFCb(self, msg):
        ...
        if (pt.transforms[0].translation.x < -0.39 and pt.transforms[0].translation.x > -0.41) and flag == 0:
            obj0.set_goal_pos_callback(225) #motor 1 first
            obj1.set_goal_pos_callback(235) #link 2 motor
            
            time.sleep(0.5)
            
            obj0.set_goal_pos_callback(245) #motor 1 first
            obj1.set_goal_pos_callback(235) #link 2 motor
        ...
        # # Start of traj takeoff
        # if (pt.transforms[0].translation.z < 0.66 and pt.transforms[0].translation.z > 0.64)\
        #     and (pt.transforms[0].translation.x > 0.79 and pt.transforms[0].translation.x < 0.81 ):        
        #     obj0.set_goal_pos_callback(210) #motor 1 first
        #     obj1.set_goal_pos_callback(235) #link 2 motor
            
        # # Start of traj landing
        # if (pt.transforms[0].translation.z < 0.51 and pt.transforms[0].translation.z > 0.49)\
        #     and (pt.transforms[0].translation.x > 0.6 and pt.transforms[0].translation.x < 0.90 ):       
        #     obj0.set_goal_pos_callback(225) #motor 1 first
        #     obj1.set_goal_pos_callback(235) #link 2 motor
            
        
        # # End of Traj
        # if (pt.transforms[0].translation.z < 0.71 and pt.transforms[0].translation.z > 0.69)\
        #     and (pt.transforms[0].translation.x > -0.81 and pt.transforms[0].translation.x < -0.79 ):        
        #     obj0.set_goal_pos_callback(225) #motor 1 first
        #     obj1.set_goal_pos_callback(235) #link 2 motor
            


        # if (pt.transforms[0].translation.x < -0.19 and pt.transforms[0].translation.x > -0.21) \
        # and (pt.transforms[0].translation.y > -0.01 and pt.transforms[0].translation.y < 0.01 ) and self.sleep_2 == 0:
        #         obj0.set_goal_pos_callback(235) #motor 1 first
        #         obj1.set_goal_pos_callback(240) #link 2 motor



        # if  (pt.transforms[0].translation.x < -0.79 and pt.transforms[0].translation.x > -0.81) \
        # and (pt.transforms[0].translation.y > -0.01 and pt.transforms[0].translation.y < 0.01 ) and self.sleep_2 == 0:
